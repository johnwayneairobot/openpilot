...
class CarState(object):
  ...
  def update(self, cp):
    ...
    # 2018.09.02 johnwayneairobot change gear shifter info for kia soul
    self.shifter_PARK = self.can_define.dv["TM_GEAR"]["TM_PARK"]
    self.shifter_REVERSE = self.can_define.dv["TM_GEAR"]["TM_REVERSE"]
    self.shifter_NEUTRAL = self.can_define.dv["TM_GEAR"]["TM_NEUTRAL"]
    self.shifter_DRIVE = self.can_define.dv["TM_GEAR"]["TM_DRIVE"]
    ...
    if self.CP.carFingerprint in (CAR.SOUL): # 2018.09.04 add multiple soul because can change
        ...
        self.brake_switch = cp.vl["ENG_INFO"]['BRAKE_PRESSED'] ==2 # 2018.09.02 johnwayneairobot "2"value is brake switch ON
        self.brake_pressed = cp.vl["ENG_INFO"]['BRAKE_PRESSED'] ==2 # 2018.09.02 change for Kia soul
        self.brake_switch_prev = cp.vl["ENG_INFO"]['BRAKE_PRESSED'] == 2  # 2018.09.02 johnwayneairobot "2"value is brake switch ON
    ...
    speed = (1. - self.v_weight) * cp.vl["TM_DATA"]['VS_TCU'] * CV.KPH_TO_MS * speed_factor + \
      self.v_weight * self.v_wheel      #2018.09.02 johnwayneairobot change ENGINE_DATA to TM_DATA and VS_TCU to match KIA SOUL
    ...
    self.pedal_gas = cp.vl["ENG_INFO"]['PEDAL_GAS'] #2018.09.02 johnwayneairobot change for pedal gas
    ...
    if self.CP.carFingerprint in (CAR.SOUL, CAR.SOUL1, CAR.SOUL2):  #2018.09.04
      self.car_gas = self.pedal_gas
            ...
            self.car_gas = cp.vl["ENG_INFO"]['PEDAL_GAS'] #2018.09.02 johnwayneairobot cruise control gas not available change to pedal gas
    ...
    self.user_brake = cp.vl["BRAKE_REPORT"]['BRAKE_REPORT_operator_override']  #2018.09.02 johnwayneairobot add for Kia soul
    #self.pcm_acc_status = cp.vl["POWERTRAIN_DATA"]['ACC_STATUS']
    self.pcm_acc_status = cp.vl["SCM_BUTTONS"]['MAIN_ON']   #2018.09.02 johnwayneairobot change to UI 0x1A6 main switch
